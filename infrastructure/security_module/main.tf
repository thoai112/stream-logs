...
data "aws_iam_policy_document" "logsinstance-assume-role-policy" {
  statement {
...
data "aws_iam_policy_document" "cust_logsinstance_access" {
  statement {
...
resource "aws_iam_role" "custlogsrole" {
    name               = "custlogsrole"
    assume_role_policy = data.aws_iam_policy_document.logsinstance-assume-role-policy.json
    inline_policy {
        name   = "policy-867530231"
        policy = data.aws_iam_policy_document.cust_logsinstance_access.json
    }

}

resource "aws_iam_instance_profile" "custlogsprofile" {
  name = "custlogsprofile"
  role = "${aws_iam_role.custlogsrole.name}"
}


# ---------------------role for lambda used by log stream
